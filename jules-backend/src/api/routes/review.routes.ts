...
export async function reviewRoutes(
  ...
  fastify.get('/sessions/:sessionId/reviews', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get('/ideas/:ideaId/reviews', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id
  ...
  fastify.get('/sessions/:sessionId/reviews/summary', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id
      const { sessionId } = request.params as { sessionId: string }
