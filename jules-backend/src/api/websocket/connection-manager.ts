...
export class ConnectionManager extends EventEmitter {
  ...
  private authenticateSocket(socket: AuthenticatedSocket, data: { userId: string, token?: string }): void {
    try {
  ...
  getConnectionStats(): any {
    return {
      ...
      connectionsBySession: Object.fromEntries(
        Array.from(this.sessionConnections.entries()).map(([sessionId, sockets]) => [
          sessionId,
          {
