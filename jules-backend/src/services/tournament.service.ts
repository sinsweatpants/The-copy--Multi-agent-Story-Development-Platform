import { PrismaClient, Tournament, TournamentTurn, Review, Idea } from '@prisma/client'
import { AgentService } from './agent.service'
import { PromptBuilder } from '../integrations/gemini/prompts'
import { logger } from '../utils/logger'
import { TournamentResult, TournamentTurnResult } from '../types/tournament.types'
...
export class TournamentService {
    ...
    try {
      ...
      for (const [ideaId, ideaReviewsList] of ideaReviews) {
        if (ideaReviewsList.length > 0) {
          const averageScore = ideaReviewsList.reduce(
  ...
  private async executeTournamentTurn(
    ...
    try {
      ...
      const participatingAgents = await this.selectTurnAgents(turnNumber, ideas[0].sessionId)
  ...
  private async selectTurnAgents(turnNumber: number, sessionId: string): Promise<any[]> {
    ...
    const agentTypesForTurn = turnStrategies[turnNumber as keyof typeof turnStrategies] || 
    ...
    if (selectedAgents.length < 2) {
      const remainingAgents = allAgents.filter(agent => 
        !agentTypesForTurn.includes(agent.agentType)
      )
      selectedAgents.push(...remainingAgents.slice(0, 2 - selectedAgents.length))
    }
  ...
  private async executeAgentArgument(
    ...
    try {
      ...
      const parsedArgument = this.parseArgumentResponse(argumentResult.response)
  ...
  private parseArgumentResponse(response: string): {
    ...
    try {
      ...
      // Extract stance
      const stanceMatch = response.match(/(?:موقفي|My stance|Position)[:\s]*(.+?)(?:\n|$)/i)
      if (stanceMatch) {
        result.stance = stanceMatch[1].trim()
      }

      // Extract reasoning
      const reasoningMatch = response.match(/(?:حجتي|My argument|Reasoning)[:\s]*([\s\S]*?)(?:\n(?:الأدلة|Evidence|الدليل)|$)/i)
      if (reasoningMatch) {
        result.reasoning = reasoningMatch[1].trim()
      }

      // Extract evidence
      const evidenceMatch = response.match(/(?:الأدلة|Evidence)[:\s]*([\s\S]*?)(?:\n(?:الرد|Counter|Response)|$)/i)
      if (evidenceMatch) {
        result.evidence = this.parseList(evidenceMatch[1])
      }

      // Extract counter-arguments
      const counterMatch = response.match(/(?:الرد على الحجج|Counter-arguments|Response)[:\s]*([\s\S]*?)$/i)
      if (counterMatch) {
  ...
  async getTournamentSummary(tournamentId: string): Promise<any> {
    ...
    for (const argument of allArguments) {
      ...
      agentParticipation.set(agentType, (agentParticipation.get(agentType) || 0) + 1)
